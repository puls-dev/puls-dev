...
export class SQSBuilder extends BaseBuilder {
  ...
  fifo(enabled: boolean = true) {
    this._fifo = enabled;
    return this;
  }
  deduplication(enabled: boolean = true) {
  ...
  private async discoverQueue(name: string): Promise<any> {
    ...
    try {
      const urlResult = await sqs.send(
        new GetQueueUrlCommand({ QueueName: qName }),
      );
  ...
  async deploy() {
    ...
    if (dryRun) {
      const dlqName = this._dlqName
        ...
        : undefined;
      console.log(
        `   📝 [PLAN] ${existing ? "Update" : "Create"} queue "${queueName}"`,
      );
      console.log(`      └─ Type: ${this._fifo ? "FIFO" : "Standard"}`);
      console.log(
        `      └─ Visibility timeout: ${this._visibilityTimeout}s | Retention: ${this._retentionDays}d`,
      );
      if (this._delaySeconds)
        console.log(`      └─ Delivery delay: ${this._delaySeconds}s`);
      if (dlqName)
        console.log(
          `      └─ DLQ: ${dlqName} (after ${this._dlqMaxReceives} receives)`,
        );
    ...
    if (existing) {
      ...
      const arnResult = await sqs.send(
        new GetQueueAttributesCommand({
          QueueUrl: this.resolvedUrl,
          AttributeNames: [QueueAttributeName.QueueArn],
        }),
  ...
  async destroy() {
    ...
    if (!existing) {
      console.log(
        `   ✅ Queue "${this.queueName()}" does not exist - nothing to do`,
      );
    ...
    await getSQSClient().send(
      new DeleteQueueCommand({ QueueUrl: this.resolvedUrl! }),
    );
