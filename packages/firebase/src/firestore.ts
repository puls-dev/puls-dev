...
export class FirebaseFirestoreBuilder extends BaseBuilder {
  ...
  index(collection: string, fields: IndexField[]) {
  ...
  private async deployRules(dryRun: boolean): Promise<void> {
    ...
    if (dryRun) {
      console.log(
        `   📝 [PLAN] Deploy Firestore rules from "${this._rulesPath}"`,
      );
      if (current)
    ...
    console.log(
      `   ✅ Rules deployed (ruleset: ${ruleset.name.split("/").pop()})`,
    );
  }
  ...
  private async listExistingIndexes(): Promise<any[]> {
    try {
      const res = await cloudFetch(
        FS_BASE,
        `/${this.dbPath()}/collectionGroups/-/indexes`,
      );
      return res?.indexes ?? [];
    } catch {
      return [];
  ...
  private async deployIndexes(dryRun: boolean): Promise<void> {
    ...
    const existingKeys = new Set(
      existing.map((idx: any) => {
        ...
        const fields: IndexField[] = (idx.fields ?? [])
          ...
          .map((f: any) => ({
            field: f.fieldPath,
            order: f.order as FieldOrder,
          }));
        return this.indexKey(collection, fields);
      }),
    );

    const toCreate = this._indexes.filter(
      (i) => !existingKeys.has(this.indexKey(i.collection, i.fields)),
    );

    if (dryRun) {
      console.log(
        `   📝 [PLAN] ${toCreate.length} index(es) to create, ${this._indexes.length - toCreate.length} already exist`,
      );
      for (const idx of toCreate) {
        console.log(
          `      └─ ${idx.collection}: [${idx.fields.map((f) => `${f.field} ${f.order}`).join(", ")}]`,
        );
      }
      return;
    }
    ...
    for (const idx of toCreate) {
      ...
      console.log(
        `   ✅ Index created: ${idx.collection} [${idx.fields.map((f) => `${f.field} ${f.order}`).join(", ")}]`,
      );
    }
  ...
  async destroy() {
    ...
    if (dryRun) {
      if (this._rulesPath)
        console.log(
          `   📝 [PLAN] Rules release cannot be rolled back via API - do this in the Firebase console`,
        );
      console.log(`   ℹ️  Firestore databases cannot be deleted via API`);
    } else {
      console.log(
        `   ℹ️  Firestore databases cannot be deleted via API. Remove rules/indexes manually in the Firebase console.`,
      );
    }
