...
export class FargateBuilder extends BaseBuilder {
  ...
  private async discoverService(name: string): Promise<any> {
    try {
      ...
      const clusterDesc = await ecs.send(
        new DescribeClustersCommand({ clusters: [this._clusterName] }),
      );
      ...
      if (
        e.name === "ClusterNotFoundException" ||
  ...
  private async ensureCluster(): Promise<string> {
    ...
    const created = await ecs.send(
      new CreateClusterCommand({ clusterName: this._clusterName }),
    );
  ...
  private async discoverDefaultVpc(): Promise<{
    vpcId: string;
    subnetIds: string[];
  }> {
    ...
    if (!vpcId)
      throw new Error(
        `[Fargate:${this.name}] No default VPC found. Use .subnets(ids[]) to specify subnets.`,
      );
  ...
  private async resolveNetworking(): Promise<{
    subnetIds: string[];
    ...
    const sgId =
      this._securityGroupIds?.[0] ?? (await this.ensureSecurityGroup(vpcId));
    return {
  ...
  async deploy() {
    ...
    if (dryRun) {
      console.log(
        `   📝 [PLAN] ${existing ? "Update" : "Create"} Fargate service "${this.name}"`,
      );
      console.log(`      └─ Cluster: ${this._clusterName}`);
      console.log(`      └─ Image: ${this._image}`);
      console.log(
        `      └─ CPU: ${this._cpu} | Memory: ${this._memory}MB | Replicas: ${this._replicas}`,
      );
    ...
    const [clusterArn, executionRoleArn, logGroupName, networking] =
      await Promise.all([
        this.ensureCluster(),
    ...
    if (Object.keys(this._env).length) {
      ...
      containerDef.environment = Object.entries(resolvedEnv).map(
        ([name, value]) => ({ name, value }),
      );
    }

    const taskDef = await getECSClient().send(
    ...
    console.log(
      `   ✅ Registered task definition: ${this.name}:${taskDef.taskDefinition!.revision}`,
    );
    ...
    if (existing) {
      ...
      console.log(
        `   ✅ Updated service "${this.name}" → rolling deployment started`,
      );
    } else {
      const created = await ecs.send(
      ...
      console.log(
        `🚀 Created Fargate service "${this.name}" (arn=${this.resolvedArn})`,
      );
    }
  ...
  async destroy() {
    ...
    if (!existing) {
      console.log(
        `   ✅ Service "${this.name}" does not exist - nothing to do`,
      );
    ...
    if (dryRun) {
      console.log(
        `   📝 [PLAN] Drain and delete service "${this.name}" in cluster "${this._clusterName}"`,
      );
      return { destroyed: this.name };
    }
