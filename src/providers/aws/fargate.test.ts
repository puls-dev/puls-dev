...
describe('FargateBuilder Unit Tests', () => {
  ...
  beforeEach(() => {
    ...
    ecsCalls = []; ec2Calls = []; iamCalls = []; cwCalls = [];
    mockEcsResponses = {}; mockEc2Responses = {}; mockIamResponses = {}; mockCwResponses = {};
  ...
  test('discovers existing service when cluster and service are active', async () => {
    mockEcsResponses['DescribeClustersCommand'] = {
      clusters: [{ status: 'ACTIVE', clusterArn: 'arn:aws:ecs:us-east-1:123:cluster/puls' }],
    };
    mockEcsResponses['DescribeServicesCommand'] = {
      services: [{ status: 'ACTIVE', serviceArn: 'arn:aws:ecs:us-east-1:123:service/puls/my-svc' }],
    };
  ...
  test('performs dry-run without any write API calls', async () => {
    ...
    const builder = new FargateBuilder('my-svc');
    builder.image('my-org/api:latest').cpu(512).memory(1024).port(3000).replicas(2);
    ...
    const writeCalls = [...ecsCalls, ...ec2Calls, ...iamCalls, ...cwCalls].filter(
  ...
  test('creates new service — registers task def, creates cluster, role, log group, and service', async () => {
    ...
    mockEcsResponses['RegisterTaskDefinitionCommand'] = {
      taskDefinition: { taskDefinitionArn: 'arn:aws:ecs:us-east-1:123:task-def/my-svc:1', revision: 1 },
    };
  ...
  test('updates existing service with new task definition', async () => {
    ...
    mockCwResponses['DescribeLogGroupsCommand'] = { logGroups: [{ logGroupName: '/puls/my-svc' }] };
    ...
    mockEcsResponses['RegisterTaskDefinitionCommand'] = {
      taskDefinition: { taskDefinitionArn: 'arn:aws:ecs:us-east-1:123:task-def/my-svc:2', revision: 2 },
    };
  ...
  test('drains and deletes service on destroy', async () => {
    ...
    const drain = ecsCalls.find(
      (c) => c.commandName === 'UpdateServiceCommand' && c.input.desiredCount === 0,
    );
    assert.ok(drain);
