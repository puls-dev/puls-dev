...
export class APIGatewayBuilder extends BaseBuilder {
  ...
  async deploy() {
    ...
    for (const i of existingIntegrations.Items ?? []) {
      if (i.IntegrationUri) integrationByFnArn.set(i.IntegrationUri, i.IntegrationId!);
    }
    ...
    for (const r of this._routes) {
      ...
      if (!fnArn) throw new Error(`[APIGateway:${this.name}] Lambda "${r.fn.name}" has no resolvedArn — deploy it before the API.`);
