...
export class Route53Builder extends BaseBuilder {
  ...
  private async registerDomain(dryRun: boolean) {
    ...
    if (dryRun) {
      ...
      if (c) console.log(`      └─ Registrant: ${c.FIRSTNAME} ${c.LASTNAME} <${c.EMAIL}>`);
      return;
    }
    ...
    if (!contact) throw new Error(`register() called without contact details — provide a RegistrantContact`);
