...
export class RDSBuilder extends BaseBuilder {
  ...
  private async discoverInstance(identifier: string): Promise<any> {
    try {
      const result = await getRDSClient().send(
  ...
  private async discoverDefaultVpc(): Promise<{
    ...
    if (!vpc?.VpcId)
      throw new Error(
        `[RDS:${this.name}] No default VPC found. Use .subnets(ids[]) to specify subnets.`,
      );
  ...
  private async ensureSubnetGroup(subnetIds: string[]): Promise<string> {
    ...
    try {
      await rds.send(
        new DescribeDBSubnetGroupsCommand({ DBSubnetGroupName: groupName }),
      );
      return groupName;
    } catch (e: any) {
  ...
  async deploy() {
    ...
    if (dryRun) {
      console.log(
        `   📝 [PLAN] ${existing ? "Update" : "Create"} RDS instance "${this.name}"`,
      );
      console.log(`      └─ Engine: ${this._engine} ${this._engineVersion}`);
      console.log(
        `      └─ Class: ${this._instanceClass} | Storage: ${this._storage}GB`,
      );
    ...
    if (!this._username || !this._password) {
      throw new Error(
        `[RDS:${this.name}] .credentials(username, password) is required`,
      );
    }
    ...
    if (this._subnetIds && this._securityGroupIds) {
      ...
      subnetGroupName = await this.ensureSubnetGroup(
        this._subnetIds ?? subnetIds,
      );
      securityGroupIds = this._securityGroupIds ?? [
    ...
    if (existing) {
      ...
      console.log(
        `🚀 Creating RDS instance "${this.name}" - waiting for available...`,
      );

      await this.waitFor(
        `"${this.name}" to become available`,
        async () => {
          const r = await getRDSClient().send(
            new DescribeDBInstancesCommand({ DBInstanceIdentifier: this.name }),
          );
      ...
      console.log(
        `   🌐 Endpoint: ${this.resolvedEndpoint}:${this.resolvedPort}`,
      );
    }
  ...
  async destroy() {
    ...
    if (dryRun) {
      console.log(
        `   📝 [PLAN] Delete RDS instance "${this.name}" (no final snapshot)`,
      );
      return { destroyed: this.name };
    }
