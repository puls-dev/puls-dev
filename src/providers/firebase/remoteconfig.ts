...
export class FirebaseRemoteConfigBuilder extends BaseBuilder {
  ...
  override(paramKey: string, conditionName: string, value: ParamValue) {
    ...
    if (!p)
      throw new Error(
        `[RemoteConfig] param "${paramKey}" not defined - call .param()/.bool()/etc. first`,
      );
    p.conditionalValues ??= {};
  ...
  private async rcFetch(
    ...
    if (!res.ok) {
      ...
      throw new Error(
        `RemoteConfig API ${opts.method ?? "GET"} ${path} → ${res.status}: ${body}`,
      );
    }
  ...
  async deploy() {
    ...
    if (dryRun) {
      ...
      for (const p of this._params) {
        const overrides = p.conditionalValues
          ...
          : "";
        console.log(
          `      └─ ${p.key} (${p.type}): ${serialize(p.value)}${overrides}`,
        );
      }
      if (this._conditions.length) {
        console.log(
          `   📝 [PLAN] Set ${this._conditions.length} condition(s):`,
        );
    ...
    const { data: current, etag } = await this.rcFetch(
  ...
  async destroy() {
    ...
    console.log(
      `   ℹ️  RemoteConfig templates cannot be deleted via API - clear parameters in the Firebase console`,
    );
