...
export class FirebaseHostingBuilder extends BaseBuilder {
  ...
  async deploy() {
    ...
    if (!this._sourcePath) throw new Error(`[Firebase.Hosting:${siteId}] .source("./dist") is required`);

    const files = walkDir(this._sourcePath);
    if (files.length === 0) throw new Error(`[Firebase.Hosting:${siteId}] No files found in "${this._sourcePath}"`);
    ...
    const absoluteToUrl: Record<string, string> = {};
    ...
    for (const hash of required) {
      ...
      if (!res.ok) throw new Error(`Upload failed for ${absPath}: ${res.status}`);
    }
