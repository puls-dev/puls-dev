...
export class FirebaseAuthBuilder extends BaseBuilder {
  ...
  google(creds: OAuthCredentials)    { this._providers.google    = { enabled: true, ...creds }; return this; }
  github(creds: OAuthCredentials)    { this._providers.github    = { enabled: true, ...creds }; return this; }
  facebook(creds: OAuthCredentials)  { this._providers.facebook  = { enabled: true, ...creds }; return this; }
  twitter(creds: OAuthCredentials)   { this._providers.twitter   = { enabled: true, ...creds }; return this; }
  apple(creds: OAuthCredentials)     { this._providers.apple     = { enabled: true, ...creds }; return this; }
  microsoft(creds: OAuthCredentials) { this._providers.microsoft = { enabled: true, ...creds }; return this; }

  // Domains allowed to use Firebase Auth (e.g. your app domain + localhost)
  authorizedDomains(domains: string[]) { this._authorizedDomains = domains; return this; }

  // ── Helpers ───────────────────────────────────────────────────────────────

  private projectPath() { return `/projects/${getProjectId()}`; }

  private async getConfig(): Promise<any> {
    try { return await cloudFetch(AUTH_BASE, `${this.projectPath()}/config`); }
    catch { return null; }
  }
  ...
  private async getIdp(provider: string): Promise<any> {
    try { return await cloudFetch(AUTH_BASE, `${this.projectPath()}/defaultSupportedIdpConfigs/${IDP_ID[provider]}`); }
    catch { return null; }
  }

  private async upsertIdp(provider: string, creds: OAuthCredentials & { enabled?: boolean }): Promise<void> {
  ...
  async deploy() {
    ...
    const oauthEntries = Object.entries(oauthProviders) as [string, OAuthCredentials & { enabled?: boolean }][];

    if (dryRun) {
      if (hasBuiltIn) {
        console.log(`   📝 [PLAN] Configure sign-in methods:`);
        if (emailPassword) console.log(`      └─ Email/Password (passwordRequired: ${emailPassword.passwordRequired ?? true})`);
    ...
    if (hasBuiltIn || this._authorizedDomains) {
      ...
      if (this._authorizedDomains) console.log(`   ✅ Authorized domains set: [${this._authorizedDomains.join(', ')}]`);
    }
